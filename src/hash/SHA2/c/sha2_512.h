 ...
 *
 * Implementation of the sha2_512 hash algorithm per RFC 4634. See docs for the
 * specification.
 *
 *
 * Usage:
 ...
 * struct sha2_384
 *
 * uint8_t digest[64]   -- public; digest after finalization
 *
...
/*
 * sha2_512 sha2_512_core
 *
 * Core of sha2_512 hash function; operates on a single round from m->partial
...
/*
 * sha2_512 sha2_512_init
 *
 * Initializes sha2_2512 struct with initial state seed, empties partial and digest
 *
 *
...
/*
 * sha2_512 sha2_512_update
 *
 * Updates the state of the sha2_512 struct with new values
*/
                                   ...
                                   uint64_t len)
{
    ...
    for (i = 0; i < len; i++) {
        if (m->p_len == 128) {
            ...
            // Once we finish a buffer, call the core sha2_512 function to update
...
/*
 * sha2_512 sha2_512_finalize
 *
 * Finalizes the sha2_512 structure; pads the partial block as necessary. Also
