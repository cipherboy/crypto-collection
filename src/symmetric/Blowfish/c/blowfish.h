 ...
 *
 * Implementation of the blowfish encryption algorithm per Springer blowfish
 * paper. See docs for the specification.
*/

#pragma once
#ifndef CC_BLOWFISH_H
#define CC_BLOWFISH_H
                                    ...
                                    uint32_t* right)
{
    ...
    for (i = 0; i < 16; i++) {
        *left = (*left) ^ bf->P[i];
        tmp = blowfish_feistel(bf, *left) ^ (*right);
        *right = *left;
        *left = tmp;
    }
    tmp = *right;
                                 ...
                                 size_t key_len)
{
    ...
    for (i = 0; i < 18; i++) {
        bf->P[i] = blowfish_initial_subkeys[i];
    }
    for (i = 0; i < 1024; i++) {
        bf->S[i] = blowfish_initial_sbox[i];
    }
    for (i = 0; i < 18; i++) {
        bf->P[i] = bf->P[i] ^ key[k];
        k = (k + 1) % key_len;
